...
import type { ResponseWell, Well } from '@/types/well/WellExplorer';
...
export default function useModelWells({ regions, customModelDetail }: Props) {
  ...
  const wType = customModelDetail.welltype_scenario.name.includes('Irrigation')
    ? 0
    : 1;
  ...
  useEffect(() => {
    async function getWells() {
      ...
      for (const queryParams of getWellsParams) {
        console.time('fetch');
        const res = await fetch(`${apiRoot}/api/well_explorer/region_wells/`, {
          method: 'POST',
      ...
      tempWells = tempWells.filter((well) => {
        if (
          customModelDetail.depth_range_min &&
          well.depth < customModelDetail.depth_range_min
        )
          return false;
        if (
          customModelDetail.depth_range_max &&
          well.depth > customModelDetail.depth_range_max
        )
          return false;
        if (
          customModelDetail.unsat_range_min &&
          well.depth < customModelDetail.unsat_range_min
        )
          return false;
        if (
          customModelDetail.unsat_range_max &&
          well.depth > customModelDetail.unsat_range_max
        )
          return false;
        return true;
      });
  ...
  const getWellsByAgeThres = async (agethres: number) => {
    ...
    for (const queryParams of getWellsParams) {
      ...
      const res = await fetch(
        ...
        {
          method: 'POST',
          body: JSON.stringify({ ...queryParams, agethres }),
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Token ${auth.token}`,
          },
